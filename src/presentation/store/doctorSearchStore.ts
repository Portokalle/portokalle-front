...
interface DoctorSearchState {
  ...
  fetchDoctors: (service: IDoctorSearchService) => Promise<void>;
  reset: () => void;
  clearResults: () => void;
...
export const useDoctorSearchStore = create<DoctorSearchState>((set, get) => {
  const debouncedFetchDoctors = debounce(async (service: IDoctorSearchService) => {
    const { searchTerm } = get();
    const normalizedSearchTerm = searchTerm.trim().toLowerCase();

    set({ loading: true, error: null });
    try {
      const doctorsByName = await service.fetchDoctors(normalizedSearchTerm, SearchType.Name);
      const doctorsBySpecializations = await service.fetchDoctors(normalizedSearchTerm, SearchType.Specializations);
      ...
      if (!normalizedSearchTerm || normalizedSearchTerm.length < 4) {
        ...
        uniqueDoctors = await service.fetchDoctors("", SearchType.Name);
      }
  ...
  return {
    ...
    fetchDoctors: (service) => new Promise<void>((resolve) => {
      debouncedFetchDoctors(service);
      resolve();
    }),
